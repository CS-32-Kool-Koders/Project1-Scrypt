...
int main()
{
    ...
    try
    {
        ...
        // std::cout << "TEST: " << astNodes[1]->blocklist[1]->condition << std::endl;
        ...
        for (Blocks *rootBlock : astNodes)
        {
            if (!rootBlock->blocklist.empty())
            {
                std::cout << "RootBlock type: " << rootBlock->type << std::endl;
                if (rootBlock->condition)
                {
                    for (Tokens token : rootBlock->condition->getTokens())
                    {
                        std::cout << token.text;
                    }
                    std::cout << std::endl;
                }
                for_each(rootBlock->blocklist.begin(), rootBlock->blocklist.end(), [](const auto &elem)
                         { std::cout << elem->type << " owo "; });
                // continue;
            }

            std::cout << "end of for loop: " << rootBlock->type << std::endl;
        }
        // for (Blocks *rootBlock : astNodes)
        // {
